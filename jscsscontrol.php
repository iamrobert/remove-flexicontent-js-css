...
if(!empty($remove_flexi))
			{
				//REMOVE FC GET
				$flexi_pattern = '@<head>.*<script type="text/javascript">.*(var.*_FC_GET.*((?:[a-z][a-z]+.))"};).*</script>.*</head>@isU';
...
$flexi_pattern2 = '@<head>.*(<script type="text/javascript">.\s*\(function\(\){Joomla.JText.*</script>).*</head>@isU';
				...
				//REMOVE FANCYBOX JCE EDITOR
				$jce_pattern = '@<head>.*<script type="text/javascript">.*(.*({popup:{((?:[a-z][a-z]+.*video"\}\)\;)))).*</script>.*</head>@isU';
				$this->removeInlineJavaScript($body, $jce_pattern);
			}
