...
class PlgSystemJsCssControl extends JPlugin
{
    ...
    public function onBeforeCompileHead()
    {
        if(!empty($this->execute))
        {
            ...
            if(!empty($js) OR !empty($css) OR !empty($remove_jcaption) OR !empty($remove_flexi))
            {
    ...
    public function onAfterRender()
    {
        ...
        if(!empty($this->execute))
        {
			...
			OR !empty($remove_flexi))
            {
                // Remove JS and CSS from output
                $body = JFactory::getApplication()->getBody();
                
                /* to remoe extra google map api */
                $body = preg_replace('@<script.*src="https://maps.google.com/maps/api/js\?libraries=geometry,places&amp;language=en" .*></script>@isU', "", $body);

                if(!empty($this->exclude_js_files))
                {
                    preg_match_all('@<script[^>]*src=["|\'][^>]*\.js(\?[^>]*)?["|\'][^>]*/?>
.*</script>@isU', $body, $matches_js);
                ...
                if(!empty($remove_jcaption))
                {
                    preg_match('@
                    <head>
.*<script type="text/javascript">.*(jQuery\(window\)\.on\(\'load\',\s*function\(\)\s*{\s*\n?\s*new JCaption\(["|\']img.caption["|\']\);.*}\);).*</script>.*
</head>
@isU', $body, $match_jcaption);
 ...
 preg_match('@<head>.*(<script type="text/javascript">.\s*\(function\(\) {\n.*Joomla.JText.load.+\s.*</script>).*</head>@isU', $body, $match_flexi);
				...
				/* var _FC_GET = {"ilayout":"articles","flexi_callview":"module","view":"item","option":"com_flexicontent","isflexicontent":"yes"}; */
				
                if(!empty($remove_tooltip))
                {
                    preg_match('@
                <head>
.*<script type="text/javascript">.*(jQuery\(document\)\.ready\(function\(\){\s*\n?\s*jQuery\(\'\.hasTooltip\'\)\.tooltip.*}\);.*}\);).*</script>.*
</head>
@isU', $body, $match_tooltip);
