	...
	public function onBeforeCompileHead()
	{
		if(!empty($this->execute))
		{
			...
			if(!empty($remove_flexi))
			{
				...
				$this->exclude_css_files[] = 'components/com_flexicontent/librairies/select2/select2.css';
				$this->exclude_css_files[] = 'components/com_flexicontent/assets/css/flexicontent.css';
...
$flexi_pattern1 = '@<head>.*(<script type="text/javascript">.\s*\(function\(\) {\n.*Joomla.JText.load.+\s.*</script>).*</head>@isU';
				...
				//REMOVE FLATTENED JTEXT LOAD
$flexi_pattern2 = '@<head>.*(<script type="text/javascript">.\s*\(function\(\){Joomla.JText.*</script>).*</head>@isU';
